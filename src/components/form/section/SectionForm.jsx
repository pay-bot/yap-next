...
function SectionForm({ defaultValues, onFormSubmit }) {
  ...
  const removeMedia = async () => {
    return axios
      .delete(
        `${process.env.REACT_APP_API_URL}/pages/${pageId}/sections/${defaultValues.id}/removeMedia`,
        {}
      )
          ...
          <form onSubmit={onSubmit}>
                    ...
                    <div className="mt-6 w-6/12">
                      {defaultValues?.media && (
                        <div className="w-40  border">
                          <button
                            type="button"
                            onClick={removeMedias}
                            className="flex w-full justify-end bg-gray-300 p-1"
                          >
                            <Delete variant="bordered" />
                          </button>
                          <img
                            src={`http://127.0.0.1:8000/uploads/images/${defaultValues?.media}`}
                            alt=""
                      ...
                      <QuillWrapper
                        value={
                          defaultValues?.content
                            ? editorContent || defaultValues?.content?.content
                            : editorContent || ""
                        }
                        onChange={onEditorStateChange}
                      />

                      <span className="absolute top-0 left-4 -mt-3 rounded-sm bg-black p-1 text-sm text-white">
                        Content
                      </span>
                    </div>
                  </div>
            ...
            <TabPanel value="2">
                ...
                <div className="w-6/12">
                  {defaultValues?.media && (
                    <div className="">
                      ...
                      <img
                        src={`${process.env.REACT_APP_API_ASSET_URL}/uploads/images/${defaultValues?.media}`}
                        alt=""
              ...
              <div className="w-full">
                <QuillWrapper
                  value={
                    defaultValues?.content_in
                      ? editorContentEn || defaultValues?.content?.content_in
                      : editorContentEn || ""
                  }
                  // value={editorContent}

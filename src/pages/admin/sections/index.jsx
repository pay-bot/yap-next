          ...
          <Droppable droppableId="sections">
            {(provided) => (
              <ul className="space-y-5" {...provided.droppableProps} ref={provided.innerRef}>
                {sor?.map((page, index) => (
                  <Draggable key={page.id} draggableId={`${page.id}`} value={page.id} index={index}>
                    {(provided) => (
                      <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                        <div key={page.id} className="bg-white border shadow rounded  ">
                          ...
                          <div className="w-full p-3">
                            <img src={`${page.url ?? 'https://admin.bzpublish.com/img/bz-publish.svg'}`} alt="" className="w-full" />
                          </div>
                        </div>
                      </div>
                    )}

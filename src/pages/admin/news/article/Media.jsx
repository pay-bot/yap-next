...
export default function Media() {
  ...
  media?.data?.data?.map((data) => {
    if (data.photoable_id.toString() === articleId.toString() && data.photoable_type === 'App\\Models\\Article') {
  ...
  return (
    <SectionWrapper>
      <ContentHeading title="Media List">
        <button type="button" onClick={() => dispatch(openModal({ componentName: 'AttachMediaArticle', id: '' }))}>
          Attach Media
        </button>

        <ModalWrapper componentName="AttachMediaArticle" modalid="">
          <AttachMediaForm dataMedia={dataMedias} photoable_id={articleId} photoable_type="App\Models\Article" httpPost="/resources/photos/attach" />
        </ModalWrapper>
      </ContentHeading>

      <AddMediaWrapper httpPost="resources/photos/upload" photoable_id={articleId} photoable_type_name="Article" photoable_type="App\Models\Article" />

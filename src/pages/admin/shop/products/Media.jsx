...
export default function Media() {
  ...
  media?.data?.data?.map((data) => {
    if (data.photoable_id.toString() === productId.toString() && data.photoable_type === 'App\\Models\\Product') {
  ...
  return (
    <SectionWrapper>
      <ContentHeading title="Media List">
        <button type="button" onClick={() => dispatch(openModal({ componentName: 'AttachMediaProduct', id: '' }))}>
          Attach Media
        </button>

        <ModalWrapper componentName="AttachMediaProduct" modalid="">
          <AttachMediaForm dataMedia={dataMedias} photoable_id={productId} photoable_type="App\Models\Product" httpPost="/resources/photos/attach" />
        </ModalWrapper>
      </ContentHeading>

      <AddMediaWrapper httpPost="resources/photos/upload" photoable_id={productId} photoable_type_name="Product" photoable_type="App\Models\Product" />

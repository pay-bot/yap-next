    ...
    <div key={data.id} className=" border-b-2 p-3 bg-white">
      <div className="flex items-center">
        <div className="space-y-2">
          ...
          <div className="3xl:text-2xl text-xl font-bold">
            {ReactHtmlParser(data?.heading?.slice(0, 60))}
          </div>
          <div className="text-gray-400">
            {ReactHtmlParser(data?.content?.slice(0, 60))}
          </div>
          {data?.content_category &&
        ...
        <div className="ml-auto flex gap-x-1">
          ...
          <Remove
            tooltip={`Remove ${data.heading}`}
            onClick={() => removeContents(data.id)}
          />
          <Delete
            tooltip={`Delete ${data.heading}`}
            onClick={() => deleteContents(data.id)}
          />
        </div>
      </div>
    </div>

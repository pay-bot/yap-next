...
function HeaderMenu({ route }) {
  ...
  return (
    <>
      <div
        key={route.id}
        className="my-2 w-6/12 items-center rounded-md bg-white"
      >
        <div
          className={` w-full items-center rounded-md border-b border-blue-400 bg-white px-6 py-1 ${
            isMenuOpen === true ? "bg--600 " : ""
          }`}
        >
          <div key={route.id} className="flex items-center py-1">
            ...
            <div className="ml-auto flex justify-end gap-x-1">
              {/* <Icon>menu</Icon> */}
              <FontAwesomeIcon
                title={`Add Children${route.name}`}
                onClick={() =>
                  dispatch(
                    openModal({ componentName: "AddChildMenu", id: route.id })
                  )
                }
                icon="fas fa-bars"
              />
              <div className="realative">
                <FontAwesomeIcon
                  onClick={() => [
                    !uniqueNames
                      ? removeNav(route.id)
                      : handleClickParent(route.id),
                  ]}
                  icon="fas fa-trash"
                />
                {anchorEl.isParent && anchorEl.id === route.id && (
                  <div className="absolute rounded-md border-2 border-red-600 bg-white p-1">
                    You cannot delete menu with child
                  </div>
                )}
              </div>
              <FontAwesomeIcon
                onClick={() =>
                  dispatch(
                    openModal({ componentName: "EditMenu", id: route.id })
                  )
                }
                icon="fas fa-edit"
              />
          ...
          <div className=" w-6/12 gap-x-1 ">
            {navs?.data.map((data) => {
              if (data.parent_id === route.id) {
                return (
                  <div
                    className={`my-1 flex gap-x-1 rounded-md border-b border-green-400 bg-white py-1 px-6 ${
                      sidebar ? "ml-8" : "ml-1"
                    }`}
                  >
